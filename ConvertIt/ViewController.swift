...
//  ConvertIt
//
//  Created by Nick Haidari on 9/25/19.
...
class ViewController: UIViewController {
    ...
    let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0 / 0.62137}),
                         Formula(conversionString: "kilometers to miles", formula: {$0 * 062137}),
                         ...
                         Formula(conversionString: "quarts to liter", formula: {$0 / 1.05669}),
    ...
    func calculateConversion() {
        ...
        // test for the input that would make our function fail, otherwise, continue
        guard let inputValue = Double(userInput.text!) else {
            if userInput.text != "" {
                showAlert(title: "Cannot Covnvert Value", message: "\"\(userInput.text!)\" is not a valid number!")
            }
                return
        }
